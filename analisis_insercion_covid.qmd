  ...
  pdf:
    ...
    include-in-header: 
      text: |
        ...
        \pagestyle{fancy}
        \fancyhf{}
        \rhead{Inserción Laboral Post-COVID}
        \lhead{Carlos Pérez, Daniel Limón, Joaquín Vidal}
        \cfoot{\thepage}
        \usepackage{graphicx}
execute:
  warning: false
...
\begin{titlepage}
    ...
    % --- BLOQUE DE LOGOS (Lado a Lado) ---
    \begin{minipage}{0.45\textwidth}
        \centering
        % Ajusta el nombre del archivo si es necesario
        \includegraphics[height=3.5cm]{img/logo_us.png} 
    \end{minipage}
    \hfill
    \begin{minipage}{0.45\textwidth}
        \centering
        % Ajusta el nombre del archivo si es necesario (ojo a la extensión jpg/png)
        \includegraphics[height=3.5cm]{img/logo_complu.png} 
    \end{minipage}
    
    \vspace{2.5cm} % Espacio vertical
    
    % --- TÍTULO ---
    {\Huge \textbf{Impacto de la Adaptación Pedagógica COVID-19 en la Inserción Laboral de los Graduados Universitarios Andaluces} \par}
    
    \vspace{2cm}
    
    % --- AUTORES ---
    {\Large \textbf{Carlos Pérez} \\ \small Universidad Complutense de Madrid (SICUE) \par}
    \vspace{0.5cm}
    {\Large \textbf{Daniel Limón} \\ \small Universidad de Sevilla \par}
    \vspace{0.5cm}
    {\Large \textbf{Joaquín Vidal} \\ \small Universidad de Sevilla \par}
...
# Resumen {-} 
Este estudio analiza el impacto de la adaptación pedagógica forzosa por la pandemia de COVID-19 en la calidad de la inserción laboral de 58.703 egresados de universidades públicas andaluzas. Utilizando datos administrativos de la cohorte Pre-COVID (2018-2019) y las cohortes Post-COVID (2019-2023), examinamos la probabilidad de experimentar precariedad (definida como jornada parcial). 

El análisis descriptivo agregado inicial mostró una sorprendente estabilidad en las tasas de precariedad (~36%), a pesar de la disrupción educativa. Sin embargo, la posterior Regresión Logística Binaria, controlando rigurosamente por la Rama de Conocimiento (GRUPO) y el Rendimiento Académico, reveló la existencia de una **Paradoja de Simpson**. 

Los resultados indican que la estabilidad agregada se debió a un marcado *efecto composición*: un aumento en la proporción de egresados de ramas con menor precariedad estructural (como Salud y TIC). Una vez aisladas estas diferencias disciplinares, el efecto neto atribuible al periodo Post-COVID sobre la probabilidad de precariedad resultó ser estadísticamente nulo ($OR \approx 1$). 

Concluimos que, a pesar del cambio pedagógico masivo, el impacto de la pandemia no introdujo una penalización adicional en la inserción laboral de los graduados andaluces una vez controlado el tipo de titulación. Este hallazgo subraya la robustez de la calidad del sistema universitario andaluz frente a shocks externos, al tiempo que enfatiza la persistencia de las desigualdades de inserción laboral preexistentes entre ramas.
...
La irrupción de la pandemia de COVID-19 generó una disrupción sin precedentes en la educación superior a nivel global. El cambio forzoso y repentino a modelos de docencia online e híbrida supuso un desafío metodológico y didáctico para las universidades públicas andaluzas, afectando directamente la formación de las cohortes de graduados entre 2019-2023. 

La literatura preliminar sugiere que este cambio, al reducir el contacto presencial y las prácticas, podría haber comprometido el desarrollo de habilidades críticas no cognitivas (*soft skills*), planteando la hipótesis de un impacto negativo en la calidad de la inserción laboral de los egresados. En este contexto, el presente estudio tiene como objetivo determinar si el periodo de intervención Post-COVID ha incrementado la probabilidad de precariedad laboral (definida como empleo a jornada parcial) al primer año de egreso.

Inicialmente, el análisis descriptivo agregado mostró un resultado contraintuitivo: una notable estabilidad en la tasa global de precariedad, que se mantuvo fluctuando en un margen estrecho en torno al 36% entre las cohortes Pre- y Post-COVID. Esta estabilidad agregada podría llevar a concluir erróneamente que la pandemia no generó un impacto sistémico en el mercado laboral juvenil andaluz. Sin embargo, la caracterización de la muestra reveló una variación estructural significativa ($p < 0.001$) en la composición de los egresados por Rama de Conocimiento (GRUPO). Se observó un incremento del peso relativo de las titulaciones con históricamente baja precariedad (como Salud y TIC) y un descenso en ramas con precariedad estructural alta (como Educación).

Esta coexistencia de estabilidad en la media con cambios en la composición de la muestra sugiere la presencia de una **Paradoja de Simpson**. El hecho de que la tasa agregada se mantuviera estática, a pesar de que la muestra de egresados mejoró estructuralmente, indica que un posible efecto penalizador neto asociado a la formación COVID-19 podría haber sido enmascarado por un efecto composición positivo. Para resolver esta ambigüedad, se requiere ir más allá de los promedios simples.

El objetivo central de esta investigación es aislar el efecto neto de la intervención COVID-19. Para ello, se utiliza un Modelo de Regresión Logística Binaria que controla *ceteris paribus* por la Rama de Conocimiento (como principal variable estructural) y el Rendimiento Académico del graduado. Nuestros hallazgos del análisis inferencial confirman el escenario de la Paradoja de Simpson y demuestran que, una vez controladas las diferencias disciplinares, el efecto neto del periodo Post-COVID es estadísticamente nulo sobre la probabilidad de precariedad. Esta conclusión subraya la primacía de los determinantes estructurales (la elección de la rama de estudio) sobre los efectos de la coyuntura pedagógica en la inserción laboral.
...
Con el fin de aislar el efecto neto de la pandemia de otras variables confusoras y validar la hipótesis de la Paradoja de Simpson, se procedió al ajuste de un modelo multivariante. A continuación se detalla la especificación del modelo y la estimación de los parámetros.
...
library(gtsummary)
library(broom)
library(kableExtra) # Necesaria para fijar la posición
...
# 3. PRESENTACIÓN DE RESULTADOS (Con fijación de posición)
tabla_modelo <- modelo_logistico %>%
  ...
  modify_caption("**Resultados de la Regresión Logística (Odds Ratios)**")

# Convertimos a kableExtra y forzamos la posición 'hold_position' (H)
tabla_modelo %>%
  as_kable_extra(booktabs = TRUE) %>%
  kable_styling(latex_options = c("hold_position", "scale_down"))
```
...
La **Tabla 2** presenta los efectos estimados por la regresión logística para la probabilidad de insertarse en un empleo precario (jornada parcial) durante el primer año. El foco principal del análisis es evaluar si el periodo Post-COVID implica un riesgo diferencial de precariedad una vez controladas variables estructurales clave como la rama de conocimiento y el rendimiento académico.
...
Los coeficientes de las ramas revelan un patrón extremadamente marcado. Todas las ramas presentan OR muy inferiores a 1 respecto a la categoría de referencia (Rama 1), con valores que oscilan entre **0.68 (Rama 2) y 0.03 (Rama 6)**, todos ellos con significación estadística ($p < 0.001$). Estas diferencias representan variaciones de hasta treinta veces en el riesgo de precariedad, confirmando que la **rama de estudios** es el determinante más fuerte y persistente de la calidad de la inserción laboral.
...
Para visualizar la magnitud comparada de estos efectos y la predominancia de los factores estructurales sobre los coyunturales, la **Figura 4** presenta el *Forest Plot* de los coeficientes estimados:
...
1.  **Irrelevancia del Factor Temporal:** El efecto del COVID aparece centrado alrededor de $OR = 1$, con un intervalo que cruza ampliamente la línea de no-efecto. Esto confirma su irrelevancia estadística en el contexto multivariante.
2.  **Gradiente Académico:** Los efectos del rendimiento se agrupan cerca de $OR = 1$ pero con desplazamientos suficientes para indicar un gradiente de riesgo real, aunque limitado.
3.  **Determinismo Estructural:** Las ramas, en contraste, se encuentran muy por debajo de $OR = 1$, ocupando prácticamente toda la parte izquierda de la escala. Esto evidencia que el nivel de precariedad es un fenómeno eminentemente estructural, asociado a diferencias entre disciplinas y no al periodo temporal o a la coyuntura COVID.
...
La presente investigación se propuso evaluar el impacto neto de la adaptación metodológica Post-COVID-19 en la calidad de la inserción laboral (precariedad por jornada parcial) de los egresados de Grado. Los resultados permiten una interpretación matizada que desafía la hipótesis inicial de un deterioro generalizado.

El **análisis descriptivo agregado** reveló una sorprendente **estabilidad** en la tasa global de precariedad (~36%), lo que inicialmente podría sugerir un impacto nulo de la pandemia. Sin embargo, el **Modelo de Regresión Logística Binaria** demostró que esta estabilidad se debe a la existencia de una **Paradoja de Simpson**.

El principal hallazgo es que, una vez que el modelo controló rigurosamente por la **Rama de Conocimiento (GRUPO)** y el **Rendimiento Académico**, el *Odds Ratio* (OR) para el factor **Post-COVID vs. Pre-COVID** resultó ser cercano a la unidad (OR $\approx 1$) y **estadísticamente no significativo**.

Este resultado implica que el **efecto neto** de la adaptación pedagógica por la pandemia es **nulo** sobre la probabilidad de precariedad. La estabilidad agregada se explica por un **marcado efecto composición**—un aumento en la proporción de egresados de ramas con menor precariedad estructural (como Salud y TIC) que compensó cualquier posible deterioro subyacente. En conclusión, el factor más robusto en la determinación de la calidad de la inserción laboral es la **elección de la Rama de Conocimiento**, y no la coyuntura pedagógica temporal.

## Matices por rama y la Cuestión de las *Skills*

Si bien el efecto neto del COVID fue nulo, la alta significancia y las amplias diferencias en los OR entre las **Ramas de Conocimiento** (visualizadas en el *Forest Plot*) ilustran las persistentes desigualdades estructurales preexistentes. Las ramas con OR significativamente menores a 1 son aquellas que ofrecen mayor **protección** contra la precariedad, mientras que otras ramas con OR considerablemente mayores continúan enfrentando una vulnerabilidad estructural.

Este patrón nos lleva a plantear la pregunta clave en el debate sobre empleabilidad: **¿Significa que la adaptación *online* no afectó la calidad de la formación, o que el mercado laboral no valora los aspectos que sí fueron afectados?**

Es plausible que la transición Post-COVID haya impactado negativamente en el desarrollo de **habilidades blandas (*soft skills*)** o competencias transferibles (ej. comunicación, liderazgo) que se desarrollan mejor en el entorno presencial. No obstante, si el mercado laboral andaluz no valora o no mide estos aspectos a través de indicadores como la jornada laboral, el efecto negativo del COVID no se materializa en nuestro modelo. Este punto abre una línea de investigación futura crucial.

## Limitaciones

La principal limitación de este estudio radica en la naturaleza de los **datos administrativos** utilizados:

* **Ausencia de métricas de Competencias:** El *dataset* carece de variables sobre el desarrollo de habilidades blandas o la **calidad de la formación** en sí misma, impidiendo testar directamente la hipótesis del deterioro de *skills*.
* **Proxy Limitado:** La variable dependiente (`TJORNADAA1T3`) es un *proxy* de la precariedad que solo capta la **cantidad** de trabajo (jornada), pero no la **calidad** (salario, satisfacción o adecuación al puesto).

A la luz de estas limitaciones, y para incentivar la discusión con el profesor, planteamos las siguientes preguntas de reflexión:

### Preguntas para la Discusión

1.  **Sobre la Robustez del Sistema y las *Soft Skills***: La estabilidad global sugiere que el sistema universitario andaluz fue **robusto** frente al *shock* pedagógico. ¿Esta robustez es un reflejo de que el currículo formativo y la exigencia evaluativa es lo que realmente valoran los empleadores? O, por el contrario, ¿es un signo de que el **mercado laboral no penaliza la pérdida de *soft skills*** en el corto plazo, sino que se basa exclusivamente en la **etiqueta disciplinar** (Rama de Conocimiento)?

2.  **Sobre la Dirección de la Política Universitaria:** Nuestros resultados confirman que la **Rama de Conocimiento** es el factor más robusto contra la precariedad. Desde una perspectiva de política pública, ¿debería este hallazgo llevar a las universidades a invertir prioritariamente en la **mejora de la inserción laboral de las ramas que muestran mayor vulnerabilidad estructural** (aquellas con OR más altos), en lugar de centrar recursos en mitigar un efecto coyuntural (COVID) que demostró ser irrelevante a nivel de jornada laboral?
